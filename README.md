# Aditi Sharma

I thrive around people, enjoy solving problems, think analytically and have the patience to build things consistently. That's what led me to growth and marketing for tech startups, helping people discover products that make their work and lives more fun & efficient.

## Growth Work

- [**Drift**](https://godrift.ai) - run every organic channel for a robotics-sim CLI. Started video content early and pulled 150K+ views on X from a brand account in the first month.
- [**AlgoTest**](https://algotest.in) - Head of Content & Marketing. Drove the highest organic traffic in AlgoTest's four years, roughly 50% above a normal month, and lifted organic revenue ~36% in a quarter, while covering four roles that emptied out the month I joined.
- [**Tanya Khanijow**](https://www.instagram.com/tanyakhanijow/) - content & social lead for the travel creator. 5x'd followers on [X](https://x.com/TanyaKhanijow) and [LinkedIn](https://www.linkedin.com/in/tanya-khanijow-62a0b199/) and 4x'd impressions, enough to unlock brand collabs and X's Creator Revenue Program.
- [**Peerlist**](https://peerlist.io) - owned content and marketing as the sole growth function. Ads, and anything and everything else.
- [**Chosen**](https://chosenstore.in) - freelance across YouTube, LinkedIn, Instagram, newsletter, and Reddit in 2 months. A Short hit 250K views (+700 followers), videos averaged 3.5K with a few at 10–13K, and a Reddit AMA crossed 30K.
- [**ContextSDK / Elements**](https://contextsdk.com) - shipped 40+ content pieces end to end and 10+ ideas a week, research to writing to visuals.
- [**tickadoo**](https://www.tickadoo.com) - built the marketing strategy, brand guidelines, and buyer personas from scratch. Engagement up 35%, travel-attraction interest up 28%.
- [**Helmer**](https://helmer.plus/micro) - built GTM from scratch: market research, an acquisition-to-retention funnel, and onboarding emails reworked to actually learn who was signing up.
- [**Parseable**](https://www.parseable.com) - ran cold DM outreach through a third-party tool and activated builders for a free-tool launch.
- [**UXHacks**](https://uxhack.co) - 8 blogs and 30+ tweets a month, a TGIF quiz campaign to pull in users, and a running list of UX friction I kept fixing.
- **Upsurge** - positioned their AI product (Cosmo) off competitive research and built workflows for their QA testing.
- [**Soni Saloni**](https://www.instagram.com/sonisaloni__/) - brand manager for the creator. Closed brands like Indigo, and built the pitch decks, presentations, and a shortlist of dream brands worth chasing.
- **A D2C brand** - SEO and influencer campaigns: engagement up 30%, organic traffic 20%, Meta CTR 10%, brand mentions 15%.

Plus a WhatsApp community of ~2,000 marketers who ask each other the stuff they'd never post on LinkedIn.

## Things I've Built

Web apps I wanted and couldn't find, all made from a browser tab, no terminal.

- [**Nourish**](https://calories-tracker-orcin-nine.vercel.app) - a nutrition tracker that knows what a roti weighs, instead of making me log Indian food as a bad guess at American food. Next.js + Supabase.
- [**GMAT course in one file**](#) - a whole course in a single HTML file: flashcards, 105 questions, and shared progress synced through Supabase so my boyfriend and I study the same thing.
- [**Robotics course**](https://robotics-growth.netlify.app) - four weeks of ROS 2 and Gazebo I taught myself, shipped as a web app. You can't market to robotics engineers without knowing what a URDF is.
- [**Portfolio**](https://airwarmedd.github.io/portfolio-2026/) - a small corner of the internet for my work, writing, and videos.

## Case Studies

- [**Swym**](#) - a social assignment for the Shopify app. Audited their LinkedIn, then wrote the posts and Instagram carousels around what actually earns a stop in a crowded feed, and turned a client win into a case study.
- [**Memed / WLDD**](#) - a growth strategy for a clipping platform. A two-page overview plus a companion metrics doc that tied every input to the number it was meant to move.

## Personal

- [**Substack**](https://airwarmedd.substack.com) - I once wrote a blog, got 10 DMs about it across Twitter and Instagram, and realized people actually want to read what I write.
- [**X**](https://x.com/airwarmedd) - real time and unfiltered. Travel threads, marketing takes, half-formed ideas.
- [**LinkedIn**](https://www.linkedin.com/in/aditisharma1408/) - a bittersweet relationship. Some days I post like a professional with a full-time job, other days like a 22-year-old who just joined the platform.
- [**Instagram**](https://www.instagram.com/airwarmedd/) - experimenting with video, so you'll find random things I find interesting, things I do around Delhi (my love), and whatever I'm building.
- [**Goodreads**](https://www.goodreads.com/user/show/181212913) - in 2025 I read 25 books, almost all of them standing in the metro two hours a day with no network. Best reading time I've ever had.
